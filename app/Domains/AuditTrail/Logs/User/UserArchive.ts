...
export default class UserArchiveLog extends BaseLog {
  constructor(
  ...
  public async description(): Promise<string> {
    try {
      ...
      if (this.relatedUser && relatedId !== this.creator!.id) {
        return `${this.creatorName} has archived a user named ${this.relatedUserName}`
      } else {
        return `User ${this.creatorName} has archived a user`
      }
    } catch (err) {
      console.log(err)
      throw err
